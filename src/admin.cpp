...
void facilitiesManagementMenu(Database& db) {
    int choice;
    Admin a;
    User w;
    while (true) {
        ...
        cout << "2. View All Room Facilities" << endl;
        cout << "3. Update Facility Status" << endl;
        cout << "4. Remove Facility from Room" << endl;
        cout << "5. View Facilities by Room Type" << endl;
        cout << "0. Back to Main Menu" << endl;
        cout << "Enter choice: ";
        ...
        switch (choice) {
            case 1: {
                ...
                int room_id;
                string facility_name, description, availability;
                ...
                cout << "Facility Name (e.g., WiFi, AC, TV, Mini Bar, etc.): ";
                getline(cin, facility_name);
                
                cout << "Description (optional): ";
                getline(cin, description);
                
                cout << "Availability (Available/Not Available/Under Maintenance): ";
            ...
            case 2: {
                cout << "\n=== ALL ROOM FACILITIES ===" << endl;
                a.viewAllRoomFacilities(db);
                break;
            }
            
            case 3: {
                cout << "\n=== UPDATE FACILITY STATUS ===" << endl;
                a.updateFacilityStatus(db);
                break;
            }
            
            case 4: {
                cout << "\n=== REMOVE FACILITY ===" << endl;
                a.removeFacility(db);
                break;
            }
            
            case 5: {
                cout << "\n=== FACILITIES BY ROOM TYPE ===" << endl;
                a.viewFacilitiesByRoomType(db);
                break;
            }
...
void Admin::viewAllRoomFacilities(Database& db) {
    const char* sql = R"(
        SELECT rf.facility_id, rd.room_no, rd.room_type, rf.facility_name, 
               rf.availability, rf.description, rf.created_at
        FROM RoomFacilities rf
        JOIN RoomDetails rd ON rf.room_id = rd.room_id
        ORDER BY rd.room_no, rf.facility_name;
    )";
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        cout << left << setw(4) << sqlite3_column_int(stmt, 0)
             ...
             << setw(25) << (sqlite3_column_text(stmt, 5) ? 
                             (const char*)sqlite3_column_text(stmt, 5) : "N/A") << endl;
    }
    
    sqlite3_finalize(stmt);
    cout << string(100, '-') << endl;
    cout << "Total facilities: " << count << endl;

}

void Admin::updateFacilityStatus(Database& db) {
    viewAllRoomFacilities(db);
    ...
    cout << "New Status (Available/Not Available/Under Maintenance): ";
    getline(cin, new_status);
    
    const char* sql = "UPDATE RoomFacilities SET availability = ?, updated_at = ? WHERE facility_id = ?;";
    sqlite3_stmt* stmt;
    int rc = sqlite3_prepare_v2(db.getDb(), sql, -1, &stmt, nullptr);
    ...
    if (rc == SQLITE_OK) {
        ...
        sqlite3_bind_int(stmt, 3, facility_id);
        
        rc = sqlite3_step(stmt);
        if (rc == SQLITE_DONE) {
            cout << "Facility status updated successfully!" << endl;
        
        } else {
            cout << "Failed to update facility status!" << endl;
        }
    }
    sqlite3_finalize(stmt);
}

void Admin::removeFacility(Database& db) {
    viewAllRoomFacilities(db);
    
    int facility_id;
    char confirm;
    
    cout << "\nEnter Facility ID to remove: ";
    if (!(cin >> facility_id)) {
        cout << "Invalid Facility ID!" << endl;
        cin.clear();
        cin.ignore(10000, '\n');
    ...
    cout << "Are you sure you want to remove this facility? (y/n): ";
    cin >> confirm;
    cin.ignore();
    ...
    if (confirm == 'y' || confirm == 'Y') {
        ...
        if (rc == SQLITE_OK) {
            ...
            if (rc == SQLITE_DONE) {
                cout << "Facility removed successfully!" << endl;
               
            } else {
                cout << "Failed to remove facility!" << endl;
            }
        }
        sqlite3_finalize(stmt);
    } else {
...
void Admin::viewFacilitiesByRoomType(Database& db) {
    string room_type;
    cout << "Enter room type to view facilities: ";
    getline(cin, room_type);
    
    const char* sql = R"(
        SELECT DISTINCT rd.room_type, rf.facility_name, rf.availability, 
               COUNT(*) as room_count
        FROM RoomFacilities rf
        JOIN RoomDetails rd ON rf.room_id = rd.room_id
        WHERE rd.room_type LIKE ?
        GROUP BY rd.room_type, rf.facility_name, rf.availability
        ORDER BY rd.room_type, rf.facility_name;
    )";
    ...
    if (rc == SQLITE_OK) {
        string searchPattern = "%" + room_type + "%";
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            cout << left << setw(15) << sqlite3_column_text(stmt, 0)
                 ...
                 << setw(15) << sqlite3_column_text(stmt, 2)
                 << setw(10) << sqlite3_column_int(stmt, 3) << endl;
        }
        cout << string(80, '-') << endl;
    }
    sqlite3_finalize(stmt);
...
bool Admin::adminPower(Database &db) {
    ...
    while (true) {
        ...
        cout << "  8. Facilities Management" << endl;
        cout << string(50, '-') << endl;
        ...
        switch (choice) {
            ...
            case 8: {
            cout << "\n>>> FACILITY MANAGEMENT SELECTED <<<" << endl;
                facilitiesManagementMenu(db);
                break;
            }
